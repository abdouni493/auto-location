...
import { Language, UserSession, Property, Reservation, Client, Worker, Inspection, Damage, Maintenance, Expense, Agency } from './types';
...
const App: React.FC = () => {
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      if (dRes.data) setDamages(dRes.data.map((d: any) => { const res = d.reservations; return { id: d.id, reservationId: d.reservation_id, vehicleId: d.vehicle_id || '', name: d.name, vehicleName: res?.vehicles ? `${res.vehicles.brand} ${res.vehicles.model}` : 'Inconnu', clientName: res?.clients ? `${res.clients.first_name} ${res.clients.last_name}` : 'Inconnu', description: d.description, position: d.position, cost: d.cost, severity: d.severity, status: d.status, repairDate: d.repair_date, createdAt: d.created_at }; }));
